...
CMD bash -c ". /tmp/.helpers && $PRE_COMMANDS && \
    ...
    git clone https://github.com/EOSIO/eos ~/eosio && cd ~/eosio && git checkout release/1.8.x && git submodule update --init --recursive && \
    mkdir ~/eosio/build && cd ~/eosio/build && fold-execute cmake -DCMAKE_BUILD_TYPE='Release' -DCORE_SYMBOL_NAME='SYS' -DOPENSSL_ROOT_DIR='/usr/include/openssl' -DBUILD_MONGO_DB_PLUGIN=true $CMAKE_EXTRAS ~/eosio && \
